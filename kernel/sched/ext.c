 ...
 *
 * !NULL indicates direct dispatch is in progress. A valid pointer points to the
 * task currently being enqueued. An ERR_PTR value is used to indicate that
 * direct dispatch has already happened.
 */
...
s32 scx_bpf_dispatch(struct task_struct *p, u64 dq_id, u64 enq_flags);
			     ...
			     u64 enq_flags)
{
	...
	if (!is_local) {
		...
		if (unlikely(dq->id == SCX_DQ_INVALID)) {
			scx_ops_error("attempting to dispatch to a destroyed dq");
			/* fall back to the global dq */
...
static struct scx_dispatch_q *find_non_local_dq(u64 dq_id)
{
...
static struct scx_dispatch_q *find_dq_for_dispatch(struct rq *rq, u64 dq_id,
						   struct task_struct *p)
{
...
static s32 direct_dispatch(struct task_struct *ddsp_task, struct task_struct *p,
			   u64 dq_id, u64 enq_flags)
{
	struct scx_dispatch_q *dq;
	...
	/*
	 * SCX_DQ_LOCAL_ON is not supported during direct dispatch because
	 * dispatching to the local dq of a different CPU requires unlocking the
	 * current rq which isn't allowed in the enqueue path. Use
	 * ->select_cpu() to be on the target CPU and then SCX_DQ_LOCAL.
	 */
	if (unlikely((dq_id & SCX_DQ_LOCAL_ON) == SCX_DQ_LOCAL_ON)) {
		scx_ops_error("SCX_DQ_LOCAL_ON can't be used for direct-dispatch");
		return -EINVAL;
	}

	dq = find_dq_for_dispatch(task_rq(p), dq_id, p);
			    ...
			    int sticky_cpu)
{
	...
	if (p->scx.flags & SCX_TASK_SCD_ENQ_LOCAL) {
		enq_flags |= SCX_ENQ_SCD_LOCAL;
	...
	if (!scx_ops_enabled(p, SCX_OPSEN_EXCLUDE_DISABLING)) {
		/* when !ops, slices are refilled in put_prev_task_scx() */
		if (enq_flags & (SCX_ENQ_LAST | SCX_ENQ_SCD_LOCAL))
			goto local;
		else
			goto global;
	}
	...
	/*
	 * @p has ops enabled and dq bypass didn't trigger. Enqueue on the BPF
	 * scheduler.
	 */
	...
	*ddsp_taskp = p;
	...
	ret = scx_ops.enqueue(p, enq_flags);
	...
	*ddsp_taskp = NULL;
	...
	if (unlikely(ret < 0))
		scx_ops_error("enqueue(%s[%d], 0x%llx) failed with %d",
			      p->comm, p->pid, enq_flags, ret);
	return;

local_refill:
	p->scx.slice = SCX_SLICE_DFL;
local:
	dispatch_enqueue(&rq->scx.local_dq, p, enq_flags);
	return;

global:
	dispatch_enqueue(&scx_dq_global, p, enq_flags);
}
...
dispatch_to_local_dq(struct rq *rq, struct rq_flags *rf, u64 dq_id,
...
static void put_prev_task_scx(struct rq *rq, struct task_struct *p)
{
	...
	if (p->scx.flags & SCX_TASK_QUEUED) {
		 ...
		 * class task. Leave the task at the head of the local dq. If
		 * the BPF scheduler wants to handle this explicitly, it should
...
static void init_dq(struct scx_dispatch_q *dq, u64 dq_id)
{
...
static struct scx_dispatch_q *create_dq(u64 dq_id, int node)
{
	struct scx_dispatch_q *dq;
	int ret;

	if (dq_id & SCX_DQ_FLAG_BUILTIN)
...
static int destroy_dq(u64 dq_id)
{
	struct scx_dispatch_q *dq;
	unsigned long flags;
	int ret;
...
static s32 scx_ops_fallback_enqueue(struct task_struct *p, u64 enq_flags)
{
...
#warning "btf struct access RO only check nerfed"
/*
	if (atype == BPF_READ)
*/
 ...
 * .prep_enable() and ops.cgroup_prep_move().
 */
s32 scx_bpf_create_dq(u64 dq_id, s32 node)
{
 ...
 * spuriously. Can be called from ops.enqueue() and ops.dispatch().
 *
 * When called from ops.enqueue(), it's for direct dispatch and @p must match
 * the task being enqueued. Also, %SCX_DQ_LOCAL_ON can't be used to target the
 * local dq of a CPU other than the enqueueing one. Use ops.select_cpu() to be
 * on the target CPU in the first place.
 *
 * When called from ops.dispatch(), there are no restrictions on @p or @dq_id
...
s32 scx_bpf_dispatch(struct task_struct *p, u64 dq_id, u64 enq_flags)
{
	...
	if (ddsp_task)
		return direct_dispatch(ddsp_task, p, dq_id, enq_flags);
 ...
 * consume.
 */
bool scx_bpf_consume(u64 dq_id)
{
	struct consume_ctx *cctx = this_cpu_ptr(&consume_ctx);
	struct scx_dispatch_q *dq;
 ...
 */
s32 scx_bpf_dq_nr_queued(u64 dq_id)
{
	struct scx_dispatch_q *dq;
 ...
 * %false otherwise.
 *
 * Unavailable if ops.update_idle() is implemented and SCX_OPS_KEEP_BUILTIN_IDLE
 * is not set.
 */
 ...
 * is not set.
 */
bool scx_bpf_has_idle_cpus(void)
{
	if (!static_branch_likely(&scx_builtin_idle_enabled)) {
		scx_ops_error("built-in idle tracking is disabled");
		return false;
 ...
 */
s32 scx_bpf_destroy_dq(u64 dq_id)
{
 ...
 * callback.
 */
void scx_bpf_reenqueue_local(void)
{
	u32 nr_enqueued, i;
	struct rq *rq;
